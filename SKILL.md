...
- For a new workspace, `init` can record existing authorization and the coordinating writer, and merge the bounded [AGENTS tracking block](assets/AGENTS_tracking.md) with `--with-agents`; `--claude-bridge` optionally adds the [Claude import bridge](assets/CLAUDE_bridge.md). Existing workspaces use `enable-tracking`. Preview exact changes; preserve unrelated instructions and avoid duplicate blocks or imports. These options record permission already supplied, not new authority.
