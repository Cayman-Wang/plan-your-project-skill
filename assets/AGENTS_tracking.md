...
- 一个协调负责人写共享 STATUS，Authorization 中 `Coordinator: NAME` 至多一条且名字非空。明确交接时替换为新负责人，不并列保留旧值；有工具时用 enable-tracking --coordinator 将交接依据与旧值记入历史。已有多个负责人先报告歧义，按已确认人选修正，不自行猜选。子代理只汇报事实/证据/阻塞；写前重读最新状态，有并发变化先核对，不用旧快照覆盖。
