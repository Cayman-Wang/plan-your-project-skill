...
- `execution`: pending/running/done；`validation`: not_run/passed/failed；`acceptance`: pending/accepted/rejected；`conclusion`: not_applicable/supported/unverified。未跑、跳过、不可用都保留 not_run，在说明注明原因。
- 里程碑 evidence 是 Evidence 的 ID 数组；acceptance_basis 仅未接受时可为空。passed/failed 或 supported 必须关联至少一个 observed/historical 证据；accepted 还要求 done、passed 和非空验收依据。complete 要求全部 accepted、无活跃任务和阻塞；blocked 要求非空 blockers。
