...
- authorization 非空，记录用户决定来源、范围和限制；历史授权不覆盖较新用户指令。每项为一句话，不从计划下一步推导新的权限。`Coordinator: NAME` 是当前写入负责人的保留条目，最多一条且 NAME 非空；init/enable-tracking 的 CLI 参数用 --coordinator，不通过 --authorization 注入该条目；checkpoint 的完整状态对象直接保存该条目。没有负责人条目的兼容记录仍可读取。
...
disabled 下仅换负责人（包括修正多个旧负责人）使用此入口：先 resume 取完整 status 和双哈希，只替换 `status.authorization` 中的 `Coordinator: NAME` 为已确认的新负责人，并保存已有交接依据；在请求 changes 中明确旧/新负责人、依据及保持 disabled，其他状态原样保留。这些历史说明由请求提供，工具不会自动推导。提交后检查 tracking 仍 disabled、PLAN 未变、其他进度保留；重试复用同一 ID 和内容。不要为只换人调用 enable-tracking 或补造启用约定。
...
旧工具曾写出多个 Coordinator 时，resume/handoff 仍可只读诊断并警告，validate 拒绝将其报告为有效。已为 key_events 时可用已确认的 `enable-tracking --coordinator NAME` 预览/应用修正；disabled 时按上节 checkpoint 路径修正，保持原模式和其他进度。此命令在 disabled 下会启用记录，不能用于尚未授权启用的单纯换人。不指定负责人时不自动选择；完整 checkpoint 输入也须满足单一负责人约束。此前已给出的交接指令可直接作为依据，不另设审批步骤。
