...
同一项目只指定一个共享 STATUS 写入负责人，Authorization 的 `Coordinator: NAME` 只表示当前负责人。只交接负责人时保持原 tracking：已为 key_events 时可用 `enable-tracking --coordinator`；disabled 时通过明确请求的单次 checkpoint 替换完整状态中的 Coordinator，并在 changes 写明旧/新负责人及交接依据，保留其他进度和授权。换人不授予启用自动记录的权限。相同交接重放复用原 checkpoint ID/内容，不另造记录。旧记录已有多个负责人时也按原模式处理，先只读诊断，再按明确人选修正，不自行猜选，也不要求重复授予已有许可。子代理向负责人报告实际改动、证据、阻塞与建议下一步；不得自行重写共享计划或创建竞争入口。不同项目/独立工作流应明确各自工作区和记录归属。
