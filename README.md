...
更换负责人时，用 `enable-tracking --coordinator NAME` 预览，再携当前双哈希应用；`--authorization` 可记录已有交接指令与原因。当前 STATUS 只保留新负责人，旧值和交接依据进入历史；重复指定同一负责人不重复写入。旧记录已有两位负责人时仍可只读恢复，但须明确指定当前人选后修正。
