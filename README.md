...
更换负责人先读取当前 tracking，保持原记录模式：已为 `key_events` 时可用 `enable-tracking --coordinator NAME` 预览，再携当前双哈希应用，并用 `--authorization` 记录已有交接依据；仍为 `disabled` 时，用明确请求的单次 `checkpoint` 替换状态中的 Coordinator，在 changes 中写明旧/新负责人及交接依据，不调用启用命令。当前 STATUS 只保留新负责人，其他进度与授权边界保持不变。旧记录已有多位负责人时也按原模式修正，不自行猜选；重复相同交接不新增记录。仅换人不构成启用自动记录的授权。
